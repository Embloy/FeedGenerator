...
pub(crate) fn calc_score(job: &Job, pref: &UserPreferences) -> f64 {
    ...
    //println!("PREFS {:?} for job {}", pref, job.job_id);
...
pub(crate) fn calc_x_ranking(pref: &mut UserPreferences) {
    ...
    /*
    let mut job_type_vec: Vec<(i32, f64)> = job_types.drain().collect();
    job_type_vec.s;
    let job_type_map = job_type_vec.into_iter().collect();
    println!("MAP {:?}", job_type_map);
    pref.job_type = job_type_map;
    println!("PREFS {:?}", pref);
